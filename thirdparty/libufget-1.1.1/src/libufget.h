...
extern "C" {
    ...
    #define LIBUFGET_VERSION_STRING "1.1.1"
    #define LIBUFGET_MAJOR 1
    #define LIBUFGET_MINOR 1
    #define LIBUFGET_PATCH 1
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT uf_collection_t* uf_collection_init();
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT  uf_collection_t* uf_collection_init1(unsigned long flags);
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT uf_collection_t* uf_collection_init2(const char *baseurl, const char *cache_dir,  unsigned long flags);
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT void uf_collection_finalize(uf_collection_t * collection);
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT void uf_collection_cleanup( uf_collection_t * collection);
     ...
     * \ingroup collection
     */
    LIBUFGET_EXPORT int uf_collection_num_matrices(uf_collection_t *collection);
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT uf_matrix_t * uf_collection_get_by_id(uf_collection_t * collection, int id);
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT uf_matrix_t * uf_collection_get_by_name(uf_collection_t * collection, const char * group_name, const char *name);
     ...
     * \code{c}
     * uf_query_t * q = uf_query_sql(col, "nrows!=ncols AND isReal==1");
     * \endcode
     * \li Select all symmetric positive definite matrices:
     * \code{c}
     * uf_query_t * q = uf_query_sql(col, "nrows==ncols AND isReal==1 AND posdef==1 AND numerical_symmetry==1.0");
     * \endcode
     * \li Select all complex square matrices with more than 10000 non zero elements:
     * \code{c}
     * uf_query_t * q = uf_query_sql(col, "nrows==ncols AND isComplex==1 AND nnz>10000");
     * \endcode
     *
     * \sa uf_query_count
     * \sa uf_query_next
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT uf_query_t *uf_query_sql(uf_collection_t *collection, const char *sql);

    /**
     * \brief Use a SQL-like query to count matrices.
     * \param[in] query Query representing a selection of matrices.
     * \return number of matrices matched, or -1 on failure.
     *
     * The uf_query_count function returns the number of matrices obtained from
     * iterating over `query` using \ref uf_query_next,
     * which may be used to e.g. report progress between iterations.
     * This number is obtained by placing the WHERE clause used to create `query` in the following SQL query:
     * \code{c}
     *  SELECT COUNT(ID) FROM matrices WHERE <sql_string_used_to_create_query>;
     * \endcode
     * which is performed on the database associated to the query.
     *
     * ### Examples
     * \li Count all real rectangular matrices:
     * \code{c}
     * uf_query_t *q = uf_query_rectangular(col, UF_REAL);
     * int n = uf_query_count(q);
     * \endcode
     * \li Count all diagonal matrices:
     * \code{c}
     * uf_query_t *q = uf_query_sql(col, "upperbandwidth=0 AND lowerbandwidth=0");
     * int n = uf_query_count(q);
     * \endcode
     *
     * \sa uf_query_next
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT int uf_query_count(uf_query_t *query);
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT uf_matrix_t * uf_query_next(uf_query_t *query);
     ...
     * \ingroup query
     * */
    LIBUFGET_EXPORT void  uf_query_free(uf_query_t * query);
     ...
     * \code{c}
     * uf_query_sql(col, "nrows==ncols AND isReal==1 AND posdef==1 AND numerical_symmetry==1.0");
     * \endcode
     *
     * \sa uf_query_sql
     * \ingroup query
     * */
    LIBUFGET_EXPORT uf_query_t * uf_query_spd(uf_collection_t *collection);
     ...
     *
     * The uf_query_square function is a wrapper around one of the following function calls depending on the
     * value of field:
     * \li if field == UF_REAL:
     * \code{c}
     * uf_query_sql(col, "nrows==ncols AND isReal==1");
     * \endcode
     * \li if field == UF_COMPLEX:
     * \code{c}
     * uf_query_sql(col, "nrows==ncols AND isComplex==1");
     * \endcode
     * \li if field == UF_PATTERN:
     * \code{c}
     * uf_query_sql(col, "nrows==ncols AND isBinary==1");
     * \endcode
     *
     * \sa uf_query_sql
     * \ingroup query
     */
    LIBUFGET_EXPORT uf_query_t * uf_query_square(uf_collection_t *collection, uf_field_t field);
     ...
     *
     * The uf_query_rectangular function is a wrapper around one of the following function calls depending on the
     * value of field:
     * \li if field == UF_REAL:
     * \code{c}
     * uf_query_sql(col, "nrows!=ncols AND isReal==1");
     ...
     * \ingroup query
     */
    LIBUFGET_EXPORT uf_query_t * uf_query_rectangular(uf_collection_t *collection, uf_field_t field);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int  uf_matrix_get(uf_matrix_t * matrix);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT void uf_matrix_free(uf_matrix_t * matrix);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT void uf_matrix_print(uf_matrix_t * matrix, int brief);

    /**
     * \brief Print the meta information contained in a matrix entry from the database.
     * \param[in] stream FILE object to print to.
     * \param[in] matrix Matrix entry from the database  to print
     * \param[in] brief Indicator if the output should be brief or not.
     *
     * The uf_matrix_fprint function prints the meta information of the matrix entry to
     * stream. If the brief indicator is true (!=0) the output only contains
     * the most important information.
     *
     * \sa uf_matrix_print
     *
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT void uf_matrix_fprint(FILE * stream, uf_matrix_t * matrix, int brief);
     ...
     *
     * The uf_matrix_coord_int64 function takes a matrix entry from the database and returns the matrix in
     * coordinate storage. That means the matrix consists of three arrays of length nnz and the matrix
     * only has non zero entries at \f$ A(rowptr[i], colptr[i]) = values[i]\f$. The arrays
     * rowptr, colptr, and values are allocated by the function as a malloc-compatible memory location. Therefore,
     * they have to be freed using the \b free function after usage. The values array is either allocated as
     * an array of sizeof(double)*nnz in the case of a real or a binary matrix, or of sizeof(double complex)*nnz
     * in case of a complex matrix. The field output parameter contains again the information of the matrix read
     * having real, binary, or complex values. In order to avoid compiler warnings or errors the function should be called like
     * \code{c}
     *  uf_field_t field;
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_int64(uf_field_t *field, int64_t *nrows, int64_t *ncols, int64_t *nnz, int64_t **rowptr, int64_t **colptr, void **values, uf_matrix_t *matrix);
     ...
     * \return 0 on success, a non zero error otherwise.
     *
     * The uf_matrix_coord_int32 function takes a matrix entry from the database and returns the matrix in
     * coordinate storage. That means the matrix consists of three arrays of length nnz and the matrix
     * only has non zero entries at \f$ A(rowptr[i], colptr[i]) = values[i]\f$. The arrays
     * rowptr, colptr, and values are allocated by the function as a malloc-compatible memory location. Therefore,
     * they have to be freed using the \b free function after usage. The values array is either allocated as
     ...
     * data type of C. For 64 bit integers values use \ref uf_matrix_coord_int64 instead.
     * \remark
     * \li If the matrix was not downloaded before using \ref uf_matrix_get it will be downloaded by the call to
     * this function.
     * \li C counting is used, i.e., the row and column indices start with zero.
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_int32(uf_field_t *field, int32_t *nrows, int32_t *ncols, int32_t *nnz, int32_t **rowptr, int32_t **colptr, void **values, uf_matrix_t *matrix);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_to_csr_int32(uf_field_t field, int32_t nrows, int32_t ncols, int32_t nnz, int32_t **rowptr, int32_t **colptr, void **values);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_to_csr_int64(uf_field_t field, int64_t nrows, int64_t ncols, int64_t nnz, int64_t **rowptr, int64_t **colptr, void **values);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_to_csc_int32(uf_field_t field, int32_t nrows, int32_t ncols, int32_t nnz, int32_t **rowptr, int32_t **colptr, void **values);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_coord_to_csc_int64(uf_field_t field, int64_t nrows, int64_t ncols, int64_t nnz, int64_t **rowptr, int64_t **colptr, void **values);
     ...
     * \ingroup matrix
     * */
    LIBUFGET_EXPORT int uf_matrix_conv_memsave(int save);
